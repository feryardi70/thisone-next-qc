...
export const updateDataUjiByIdParameter = async (data: ParameterUji4th, id_parameter: string) => {
...
export const getDataUjiByIdParameter = async (id_parameter: number, signal?: AbortSignal) => {
  const response = await fetch(`${baseUrl}/qc/radiografi/parameter-uji/${id_parameter}`, { signal });
...
export const deleteDataUjiByIdParameter = async (selectedParameterId: number | null) => {
  const response = await fetch(`${baseUrl}/qc/radiografi/parameter-uji/${selectedParameterId}`, {
    method: "DELETE",
  });
