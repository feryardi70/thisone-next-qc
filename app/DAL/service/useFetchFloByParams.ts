...
//import { getDataRadForCollimationByUserIdnSNNumber } from "../repository/radiografi-repository";
import { getDataFloForCollimationByUserIdnSNNumber } from "../repository/fluoroskopi-repository";
...
export const useFetchFloByParams = ({ id_user, No_Seri, email }: Params) => {
  ...
  useEffect(() => {
    ...
    const fetchById = async () => {
      try {
        setIsLoading(true);
        const data = await getDataFloForCollimationByUserIdnSNNumber({ payloadQueryParams: { id_user: id_user as number, No_Seri } });
...
export default useFetchFloByParams;
