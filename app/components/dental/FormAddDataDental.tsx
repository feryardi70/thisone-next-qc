...
export default function AddNewDataDentalForm({ payloadQueryParams }: DentalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoading(false);
      alert("successfully Adding New Data Pesawat Sinar-X Dental");
      router.push(`/dashboard/dental/kolimasi?No_Seri=${response.data.No_Seri}&id=${response.data.id_user}`);
    } catch (error) {
      console.error(error);
      setLoading(false);
      toast("Failed to add data", {
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <main className="mt-3 flex justify-center items-center">
            <div className="px-5 py-5 shadow-md max-w-xl min-h-screen w-full border-t-4 border-cyan-500 text-black">
              <h2 className="mb-5 text-center text-3xl">Form Add Data Dental</h2>
