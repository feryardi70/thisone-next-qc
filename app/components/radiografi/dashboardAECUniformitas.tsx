...
import PerformanceChart from "../PerformanceAECUniformityData";
import Header from "../Header";
import { TriangleAlert, Plus } from "lucide-react";
import { useState } from "react";
import SpinnerCss from "../spinner-css";
import { useFetchRadMachineByUserIdnSNNumberForAECUniformitas } from "@/app/DAL/service/radiografi-service";
...
export default function DashboardRadAECUniformitas({
  payloadQueryParams,
}: DashboardRadProps) {
  const { dataUji, allDataUji, isLoading, errorMsg } =
    useFetchRadMachineByUserIdnSNNumberForAECUniformitas({ payloadQueryParams });
  ...
  const performanceData = dataUji.map(
    ({ Tanggal_uji, Uniformitas_mAs, Uniformitas_EI }) => ({
    ...
    <div className="flex flex-wrap justify-center gap-1 mb-4">
      {identifikasiPesawatUnik.map((item, index) => {
        ...
        const isActive =
          ...
          (pathname.startsWith("/dashboard/radiografi/uniformitas") &&
        ...
        <div className="flex-1 flex flex-col">
          ...
          <main className="flex-1 p-3 overflow-y-auto">
            ...
            <div className="mt-4 flex flex-col items-center p-8 gap-1">
              <h1 className="text-2xl font-bold">AEC - Densitas Standar & Uniformitas Tren</h1>
              <p>
                <small>
                  {dataUji[0]
