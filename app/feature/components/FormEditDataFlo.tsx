...
export default function EditDataFloForm({ payloadQueryParams }: RadProps) {
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    if (editResponse.status == 200) {
      ...
      router.push(`/dashboard/fluoroskopi/kolimasi?No_Seri=${editResult.data.No_Seri}&id=${editResult.data.id_user}`);
    } else {
